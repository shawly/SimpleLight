...
// With 16KB SRAM persistence (see below), we can store 31 sectors.
#define MAX_OVERLAY_SECTORS 31
...
// ----------------------------------------------------------------------------
// SRAM persistence for overlay
// Forward declaration used by SRAM restore routine
static u16 overlay_get_or_create(u32 sector);

// ----------------------------------------------------------------------------
// We reserve the top 16KB of SRAM to store overlay state across emulator runs.
// Layout:
//   u32 magic 'OVLY' | u16 version=1 | u16 count
//   repeated count times: u32 sector | 512 bytes data
// Note: 16KB allows storing up to floor((16384-8)/516) = 31 sectors.

#define SRAM_BASE ((volatile u8 *)0x0E000000)
#define SRAM_SIZE (32u * 1024u) // 32KB total
#define SRAM_OVERLAY_SIZE (16u * 1024u) // 16KB
#define SRAM_OVERLAY_OFFSET (SRAM_SIZE - SRAM_OVERLAY_SIZE)
#define OVLY_MAGIC 0x4F564C59u

static void sram_memcpy_to(volatile u8 *dst, const u8 *src, u32 size)
{
    while (size--)
    {
        *dst++ = *src++;
...
static void sram_memcpy_from(u8 *dst, const volatile u8 *src, u32 size)
{
    while (size--)
    {
        *dst++ = *src++;
...
static void overlay_save_to_sram(void)
{
    // Compute how many entries fit
    const u32 entry_size = 4u + SECTOR_SIZE;
    const u32 max_entries = (SRAM_OVERLAY_SIZE - 8u) / entry_size; // 31 with 16KB
    u16 count = 0;

    // Write header
    volatile u8 *p = SRAM_BASE + SRAM_OVERLAY_OFFSET;
    u32 magic = OVLY_MAGIC;
    u16 version = 1;
    // pack header into buffer
    u8 hdr[8];
    memcpy(hdr + 0, &magic, 4);
    memcpy(hdr + 4, &version, 2);
    memcpy(hdr + 6, &count, 2); // placeholder, will overwrite later
    sram_memcpy_to(p, hdr, 8);
    p += 8;

    // Write entries
    for (u16 i = 0; i < MAX_OVERLAY_SECTORS && count < max_entries; ++i)
    {
        if (overlay_sector_id[i] != 0xFFFF)
        {
            u32 sector = (u32)overlay_sector_id[i];
            sram_memcpy_to(p, (u8 *)&sector, 4);
            p += 4;
            sram_memcpy_to(p, overlay_sector_data[i], SECTOR_SIZE);
            p += SECTOR_SIZE;
            ++count;
        }
    }
    ...
    memcpy(hdr + 6, &count, 2);
    sram_memcpy_to(SRAM_BASE + SRAM_OVERLAY_OFFSET, hdr, 8);
}

static void overlay_load_from_sram(void)
{
    // Read header
    u8 hdr[8];
    sram_memcpy_from(hdr, SRAM_BASE + SRAM_OVERLAY_OFFSET, 8);
    u32 magic;
    u16 version;
    u16 count;
    memcpy(&magic, hdr + 0, 4);
    memcpy(&version, hdr + 4, 2);
    memcpy(&count, hdr + 6, 2);
    if (magic != OVLY_MAGIC || version != 1)
        return;

    const volatile u8 *p = SRAM_BASE + SRAM_OVERLAY_OFFSET + 8;
    // Re-apply entries
    for (u16 n = 0; n < count; ++n)
    {
        u32 sector;
        u8 buf[SECTOR_SIZE];
        sram_memcpy_from((u8 *)&sector, p, 4);
        p += 4;
        sram_memcpy_from(buf, p, SECTOR_SIZE);
        p += SECTOR_SIZE;

        if (sector >= g_sector_count)
            continue;
        // Update runtime image
        if (g_img_ram)
        {
            memcpy(g_img_ram + sector * SECTOR_SIZE, buf, SECTOR_SIZE);
        }
        // Maintain overlay map too (for consistency if g_img_ram is absent)
        u16 idx = overlay_get_or_create(sector);
        if (idx != 0xFFFF)
        {
            memcpy(overlay_sector_data[idx], buf, SECTOR_SIZE);
        }
    }
}
...
static int ensure_sd_open(void)
{
    ...
    g_sd_ready = 1;
    // Attempt to restore overlay from SRAM into runtime image/overlay map
    overlay_load_from_sram();
    return 1;
}
...
{
    if (!ensure_sd_open() || !SDbuffer)
        return RES_ERROR;
    if (count == 0)
        return RES_PARERR;
    if (address >= g_sector_count)
        return RES_PARERR;
    if ((u32)count > g_sector_count - address)
        return RES_PARERR;
    if (g_img_ram)
    {
...
u32 Write_SD_sectors(u32 address, u16 count, const u8 *SDbuffer)
{
    ...
    if (g_img_ram)
    {
        ...
        // Also record into overlay list so it can be persisted into SRAM
        u32 sector = address;
...
void ReadSram(u32 address, u8 *data, u32 size)
{
    ...
    if (address >= SRAM_SIZE)
        return;
    if (address + size > SRAM_SIZE)
        size = SRAM_SIZE - address;
    sram_memcpy_from(data, SRAM_BASE + address, size);
}
